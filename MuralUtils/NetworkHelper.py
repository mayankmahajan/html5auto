...
def getSelectionsFromNetwork(networkScreenInstance,setup,measureSelected,direction=0):
    ...
    for i in range(len(p1sel)):
        picker1['dim'].append(str(p1sel[i]).split('\n')[0])
        picker1['value'].append(str(p1sel[i]).split('\n')[1])
    ...
    for i in range(len(p2sel)):
        picker2['dim'].append(str(p2sel[i]).split('\n')[0])
        picker2['value'].append(str(p2sel[i]).split('\n')[1])
    ...
    btvSel = networkScreenInstance.btv.getAllSelections(getHandle(setup,MuralConstants.NWSCREEN,"btv"))
