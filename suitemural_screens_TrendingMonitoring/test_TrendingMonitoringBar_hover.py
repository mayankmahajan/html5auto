...
try:
    ...
    checkEqualAssert(int(view[0]),MuralConstants.BarChartIndex, str(selectedQuicklink), "","Verify View ( It should be Bar Chart)")

    p1 = TMScreenInstance.quicktrends.getColorFromBar(getHandle(setup, MuralConstants.TMSCREEN))
    chartIndex = TMScreenInstance.quicktrends.getSelectedCompareChartIndex(getHandle(setup, MuralConstants.TMSCREEN))
    compareTrend1 = TMScreenInstance.quicktrends.getColorFromBar(getHandle(setup, MuralConstants.TMSCREEN),parent="trend-compare", indexOfComp=chartIndex)
